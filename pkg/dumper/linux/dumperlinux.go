...
func getKeyringPwd() ([]byte, error) {
	...
	if len(results) == 0 {
		return nil, errors.New("secret not found in keyring")
	}
	item := results[0]
...
func getLoginData(loginDataPath string, password []byte) ([]mlogin.LoginData, error) {
	var (
	...
	for rows.Next() {
		err := rows.Scan(&url, &username, &encValue)
		if err != nil {
			return nil, err
		}
		if username == "" {
